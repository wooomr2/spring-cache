...
public class ItemSplitShardedBloomFilterCacheService implements ItemCacheService {
    ...
    private static final SplitShardedBloomFilter shardedBloomFilter = SplitShardedBloomFilter.create(
            "item-bloom-filter",
            1000,
            0.01,
            4
    ...
    public boolean supports(CacheStrategy cacheStrategy) {
        return CacheStrategy.SPLIT_SHADED_BLOOM_FILTER.equals(cacheStrategy);
    }

    @Override
    public ItemResponse read(Long itemId) {
        boolean result = splitShardedBloomFilterRedisHandler.mightContain(shardedBloomFilter, String.valueOf(itemId));
        if (!result) {
            return null;
        }
    ...
    public ItemResponse create(ItemCreateReqeust request) {
        ...
        splitShardedBloomFilterRedisHandler.add(shardedBloomFilter, String.valueOf(itemResponse.itemId()));
        return null;
    }
